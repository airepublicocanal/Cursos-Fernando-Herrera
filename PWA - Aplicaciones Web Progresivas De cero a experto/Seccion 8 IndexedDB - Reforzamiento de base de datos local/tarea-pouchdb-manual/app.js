...
db.allDocs({ include_docs: true, descending: true }).then(docs => {
    ...
    docs.rows.forEach(mensaje => console.log(mensaje.doc));
});
...
db.allDocs({ include_docs: true, descending: true }).then(docs => {
    ...
    docs.rows.forEach(mensaje => {
        console.log(mensaje.doc);
        mensaje.doc.sincronizado = true;

        db.put(mensaje.doc).then(console.log('Registro actualizado'));
        console.log(mensaje.doc);
    });
});
...
db.allDocs({ include_docs: true, descending: true }).then(docs => {
    ...
    docs.rows.forEach(mensaje => {
        ...
        if (mensaje.doc.sincronizado) {
            db.remove(mensaje.doc).then(console.log('Registro eliminado'));
        }
    });
});