...
self.addEventListener('notificationclick', e => {
    console.log('Notificación click', e);
    const notificacion = e.notification;
    const accion = e.action;

    console.log({ notificacion, accion });

    // Devuelve todos los tabs abiertos del mismo sitio
    const respuesta = clients.matchAll().then(clientes => {
        let cliente = clientes.find(c => c.visibilityState === 'visible');

        // Si el sitio esta abierto, te lleva al mismo
        if (cliente !== undefined) {
            ...
            console.log('nueva');
            // Abre una pagina desde el Service Worker
